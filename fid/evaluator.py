...
def main():
    ...
    if args.sample_batch[-4:] is not '.npz':
        if not os.path.isfile(os.path.join(args.sample_batch,'all.npz')):
            to_mat(args.sample_batch)
        args.sample_batch = os.path.join(args.sample_batch,'all.npz')
...
def to_mat(path):
    ...
    # print(path,':jpeg_files:',len(jpeg_files))
    npz_ = []
    for name in jpeg_files:
        image_path = os.path.join(path, name)
        img = np.array(Image.open(image_path).convert("RGB"),dtype='uint8')
        # print(img.shape,np.max(img),np.min(img))
        npz_.append(img)

    npz = np.stack(npz_,axis=0)
    np.savez(os.path.join(path,'all.npz'), arr_0=npz)
    print('save ' + os.path.join(path,'all.npz') + ', shape:',npz.shape)
    return
